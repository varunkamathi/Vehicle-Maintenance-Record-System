...
function VehicleInfoDisplay() {
    ...
    const fetchChallanFromBackEnd = async () => {
      try {
        const response = await axios.get(
          `/api/vehicles/get/challan/${storedUserId}`
        );
        if (response.status === 200) {
  ...
  return (
    <div className="flex flex-col min-h-screen ">
      ...
      {/* Flex container */}
      <Header />
      <div className="p-1 items-center flex-grow">
        {" "}
        {/* flex-grow ensures this section expands */}
        <div className="mb-4 mt-4 flex space-x-16 mx-[auto] max-w-[1440px] justify-center font-semibold">
          <button
      ...
      <div className="flex flex-col min-w-[1440px] self-center">
        {activeSection === "addVehicle" && (
          <VehicleInformation
            setIsAddClicked={setActiveSection}
        ...
        {activeSection === "searchChallan" && (
          <EChallanDetails
            setIsAddChallan={setActiveSection}
        ...
        {activeSection === "viewChallan" &&
          (challans.length > 0 ? (
            challans.map((challan, index) => (
              <div
                key={index}
                className="mb-6 p-4 bg-white w-full rounded-lg shadow-lg border-[1px]"
              >
            ...
            vehicles.map((vehicle, index) => (
              ...
              >
                ...
                <div className="p-4 bg-gray-50 rounded-lg">
                  <h3 className="font-semibold">
                    Owner Name:{" "}
                    {vehicle.rtoUserVehicleData.rc_owner_name || "N/A"}
                  </h3>
                  <p>VRN: {vehicle.vehicleData.registration || "N/A"}</p>
                  <p>
                    Registration date: {vehicle.rtoUserVehicleData.rc_regn_dt}
                  </p>
                  <p>
                    Vehicle fitness upto:{" "}
                    {vehicle.rtoUserVehicleData.rc_fit_upto}
                  </p>
                  <p>
                    Vehicle issue date:{" "}
                    {vehicle.rtoUserVehicleData.rc_status_as_on}
                  </p>
                  {vehicle.rtoUserVehicleData.rc_financer && (
                    <p>
                      Vehicle financer company:{" "}
                      {vehicle.rtoUserVehicleData.rc_financer}
                    </p>
                  )}
                  ...
                  <p>
                    Permanent address:{" "}
                    {vehicle.rtoUserVehicleData.rc_permanent_address}
                  </p>
            ...
            <>
              {vehicles.map((vehicle, index) => {
                ...
                return (
                  <div
                    key={index}
                    className="mb-6 p-4 bg-white w-full rounded-lg shadow-md border-[1px]"
                  >
                    <div className="p-4 bg-gray-50 rounded-lg">
                      {vehicle.rtoUserVehicleData.rc_insurance_comp &&
                        vehicle.rtoUserVehicleData.rc_insurance_upto && (
                          <>
                            ...
                            <p>
                              Insurance upto:{" "}
                              {insuranceExpiryDate.toLocaleDateString()}
                            </p>
                            {isInsuranceExpired && (
                              <p className="text-red-600 font-semibold">
                                Insurance has expired! Please renew as soon as
                                possible.
                              </p>
                            )}
                          </>
